...
End-to-end tests run with cypress. Cypress will be installed with npm automatically, but if your running on Linux you might need to install some additional libraries. Please refer to https://docs.cypress.io/guides/getting-started/installing-cypress#Linux[the official cypress documentation] on how to install these dependencies.

The End-to-end tests found in the `client/cypress` directory can be run from the `client` directory with the following commands:
